...
// TestKeyValueStoreSuite runs a suite of tests against a KeyValueStore db
// implementation.
//
...
func TestKeyValueStoreSuite(t *testing.T, newDB func() KeyValueStore) {
	t.Run("Iterator", func(t *testing.T) {
		tests := []struct {
			...
			{
				name:    "Empty db with non-existent prefix",
				content: map[string]string{},
			...
			{
				name:    "Single-item db with matching prefix",
				content: map[string]string{"key": "val"},
				prefix:  "k",
				start:   "",
				order:   []string{"key"},
			...
			{
				name:    "Single-item db with non-matching prefix",
				content: map[string]string{"key": "val"},
				prefix:  "l",
				start:   "",
				order:   nil,
			...
			{
				name:    "Multi-item db with matching prefix",
				content: map[string]string{"k1": "v1", "k5": "v5", "k2": "v2", "k4": "v4", "k3": "v3"},
				prefix:  "k",
				start:   "",
				order:   []string{"k1", "k2", "k3", "k4", "k5"},
			...
			{
				name:    "Multi-item db with non-matching prefix",
				content: map[string]string{"k1": "v1", "k5": "v5", "k2": "v2", "k4": "v4", "k3": "v3"},
				prefix:  "l",
				start:   "",
				order:   nil,
			...
			{
				name: "Multi-item db with specific prefix",
				content: map[string]string{
			...
			{
				name: "Multi-item db with non-existent prefix",
				content: map[string]string{
					"ka1": "va1", "ka5": "va5", "ka2": "va2", "ka4": "va4", "ka3": "va3",
					"kb1": "vb1", "kb5": "vb5", "kb2": "vb2", "kb4": "vb4", "kb3": "vb3",
				},
			...
			{
				name: "Multi-item db with prefix and start position",
				content: map[string]string{
					"ka1": "va1", "ka5": "va5", "ka2": "va2", "ka4": "va4", "ka3": "va3",
					"kb1": "vb1", "kb5": "vb5", "kb2": "vb2", "kb4": "vb4", "kb3": "vb3",
				},
			...
			{
				name: "Multi-item db with prefix and out-of-range start position",
				content: map[string]string{
					"ka1": "va1", "ka5": "va5", "ka2": "va2", "ka4": "va4", "ka3": "va3",
					"kb1": "vb1", "kb5": "vb5", "kb2": "vb2", "kb4": "vb4", "kb3": "vb3",
				},
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				for key, val := range tt.content {
					...
					require.NoError(t, err, "failed to insert item %s:%s into db", key, val)
				}

				// Iterate over the db with the given configs and verify the results
				var startBytes []byte
				if tt.start != "" {
	...
	t.Run("Batch", func(t *testing.T) {
		...
		// Verify data is not in db before write
		for k := range testData {
			has, err := database.Has([]byte(k))
			require.NoError(t, err, "Has operation failed for key %s", k)
			require.False(t, has, "key should not exist in db before batch write for key %s", k)
		}
		...
		// Verify data is in db after write
		for k, v := range testData {
			var val []byte
	...
	t.Run("Snapshot", func(t *testing.T) {
		...
		require.Equal(t, []byte("value4"), val, "key4 should exist in db")
