...
type BlockClock interface {
	...
	PastMedianTime(*BlockHeader) (time.Time, error)
}
...
type ConsensusDeploymentStarter interface {
	...
	HasStarted(*BlockHeader) (bool, error)
}
...
type ConsensusDeploymentEnder interface {
	...
	HasEnded(*BlockHeader) (bool, error)
}
...
// HasStarted returns true if the consensus deployment has started.
func (m *MedianTimeDeploymentStarter) HasStarted(blkHeader *BlockHeader) (bool, error) {
	switch {
...
func (m *MedianTimeDeploymentEnder) HasEnded(blkHeader *BlockHeader) (bool, error) {
	switch {
