...
func isAnnexedWitness(witness core.TxWitness) bool {
	if len(witness) < 2 {
		return false
	}
...
func CalcScriptInfo(sigScript, pkScript []byte, witness core.TxWitness,
...
// specified address.
func PayToAddrScript(addr utils.Address) ([]byte, error) {
...
func MultiSigScript(pubkeys []*utils.AddressPubKey, nrequired int) ([]byte, error) {
...
func pubKeyHashToAddrs(hash []byte, params *core.Params) []utils.Address {
	// Skip the pubkey hash if it's invalid for some reason.
	var addrs []utils.Address
	addr, err := utils.NewAddressPubKeyHash(hash, params)
	if err == nil {
...
func scriptHashToAddrs(hash []byte, params *core.Params) []utils.Address {
	// Skip the hash if it's invalid for some reason.
	var addrs []utils.Address
	addr, err := utils.NewAddressScriptHashFromHash(hash, params)
	if err == nil {
		addrs = append(addrs, addr)
	}
	return addrs
...
func ExtractPkScriptAddrs(pkScript []byte,
	chainParams *core.Params) (ScriptClass, []utils.Address, int, error) {
	...
	if data := extractPubKey(pkScript); data != nil {
		var addrs []utils.Address
		addr, err := utils.NewAddressPubKey(data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if details.valid {
		...
		addrs := make([]utils.Address, 0, len(details.pubKeys))
		for _, pubkey := range details.pubKeys {
			addr, err := utils.NewAddressPubKey(pubkey, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	if hash := extractWitnessPubKeyHash(pkScript); hash != nil {
		var addrs []utils.Address
		addr, err := utils.NewAddressWitnessPubKeyHash(hash, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if hash := extractWitnessV0ScriptHash(pkScript); hash != nil {
		var addrs []utils.Address
		addr, err := utils.NewAddressWitnessScriptHash(hash, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if rawKey := extractWitnessV1KeyBytes(pkScript); rawKey != nil {
		var addrs []utils.Address
		addr, err := utils.NewAddressTaproot(rawKey, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
