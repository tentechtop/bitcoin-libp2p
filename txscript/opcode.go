...
func opcodeCheckLockTimeVerify(op *opcode, data []byte, vm *Engine) error {
	...
	if vm.tx.TxIn[vm.txIdx].Sequence == core.MaxTxInSequenceNum {
...
func opcodeCheckSequenceVerify(op *opcode, data []byte, vm *Engine) error {
	...
	if sequence&int64(core.SequenceLockTimeDisabled) != 0 {
		return nil
	}
	...
	if txSequence&int64(core.SequenceLockTimeDisabled) != 0 {
	...
	lockTimeMask := int64(core.SequenceLockTimeIsSeconds |
		core.SequenceLockTimeMask)
	return verifyLockTime(txSequence&lockTimeMask,
		core.SequenceLockTimeIsSeconds, sequence&lockTimeMask)
}
