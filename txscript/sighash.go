...
func shallowCopyTx(tx *core.MsgTx) core.MsgTx {
	...
	txCopy := core.MsgTx{
		Version:  tx.Version,
		TxIn:     make([]*core.TxIn, len(tx.TxIn)),
		TxOut:    make([]*core.TxOut, len(tx.TxOut)),
		LockTime: tx.LockTime,
	}
	txIns := make([]core.TxIn, len(tx.TxIn))
	for i, oldTxIn := range tx.TxIn {
		txIns[i] = *oldTxIn
		txCopy.TxIn[i] = &txIns[i]
	}
	txOuts := make([]core.TxOut, len(tx.TxOut))
...
// versions.
func CalcSignatureHash(script []byte, hashType SigHashType, tx *core.MsgTx, idx int) ([]byte, error) {
...
func calcSignatureHash(sigScript []byte, hashType SigHashType, tx *core.MsgTx, idx int) []byte {
...
func calcWitnessSignatureHashRaw(subScript []byte, sigHashes *TxSigHashes,
	hashType SigHashType, tx *core.MsgTx, idx int, amt int64) ([]byte, error) {
	...
	sigHashBytes := chainhash.DoubleHashRaw(func(w io.Writer) error {
		...
		if hashType&sigHashMask != SigHashSingle &&
			...
			h := chainhash.DoubleHashRaw(func(tw io.Writer) error {
				core.WriteTxOut(tw, 0, 0, tx.TxOut[idx])
				return nil
			})
			w.Write(h[:])
		} else {
...
func CalcWitnessSigHash(script []byte, sigHashes *TxSigHashes, hType SigHashType,
	tx *core.MsgTx, idx int, amt int64) ([]byte, error) {
...
func calcTaprootSignatureHashRaw(sigHashes *TxSigHashes, hType SigHashType,
	...
	if hType&SigHashAnyOneCanPay == SigHashAnyOneCanPay {
		...
		err = core.WriteOutPoint(&sigMsg, 0, 0, &input.PreviousOutPoint)
		if err != nil {
			return nil, err
		}
		...
		if err := core.WriteTxOut(&sigMsg, 0, 0, prevOut); err != nil {
			return nil, err
		}
	...
	if hType&sigHashMask == SigHashSingle {
		...
		txOut := tx.TxOut[idx]
		if err := core.WriteTxOut(shaWriter, 0, 0, txOut); err != nil {
			return nil, err
		}
...
func CalcTapscriptSignaturehash(sigHashes *TxSigHashes, hType SigHashType,
	tx *core.MsgTx, idx int, prevOutFetcher PrevOutputFetcher,
	tapLeaf TapLeaf,
	sigHashOpts ...TaprootSigHashOption) ([]byte, error) {
