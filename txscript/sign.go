...
func RawTxInWitnessSignature(tx *core.MsgTx,
...
func WitnessSignature(tx *core.MsgTx, sigHashes *TxSigHashes, idx int, amt int64,
	subscript []byte, hashType SigHashType, privKey *btcec.PrivateKey,
	compress bool) (core.TxWitness, error) {
	...
	return core.TxWitness{sig, pkData}, nil
}
...
func RawTxInTaprootSignature(tx *core.MsgTx, sigHashes *TxSigHashes, idx int,
...
func TaprootWitnessSignature(tx *core.MsgTx, sigHashes *TxSigHashes, idx int,
	amt int64, pkScript []byte, hashType SigHashType,
	key *btcec.PrivateKey) (core.TxWitness, error) {
...
// anywhere....
func RawTxInTapscriptSignature(tx *core.MsgTx, sigHashes *TxSigHashes, idx int,
...
func RawTxInSignature(tx *core.MsgTx, idx int, subScript []byte,
...
func SignatureScript(tx *core.MsgTx, idx int, subscript []byte, hashType SigHashType, privKey *btcec.PrivateKey, compress bool) ([]byte, error) {
...
func p2pkSignatureScript(tx *core.MsgTx, idx int, subScript []byte, hashType SigHashType, privKey *btcec.PrivateKey) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subScript, hashType, privKey)
	if err != nil {
		return nil, err
	}
...
func signMultiSig(tx *core.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []utils.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
...
func sign(chainParams *core.Params, tx *core.MsgTx, idx int,
	subScript []byte, hashType SigHashType, kdb KeyDB, sdb ScriptDB) ([]byte,
	ScriptClass, []utils.Address, int, error) {
...
// versions.
func mergeMultiSig(tx *core.MsgTx, idx int, addresses []utils.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*utils.AddressPubKey)
...
// does not accept a script version, the results are undefined for other script
// versions.
func mergeScripts(chainParams *core.Params, tx *core.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []utils.Address,
...
type KeyDB interface {
	GetKey(utils.Address) (*btcec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(utils.Address) (*btcec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address utils.Address) (*btcec.PrivateKey, bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(utils.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(utils.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address utils.Address) ([]byte, error) {
	return sc(address)
}
...
// does not accept a script version, the results are undefined for other script
// versions.
func SignTxOutput(chainParams *core.Params, tx *core.MsgTx, idx int,
