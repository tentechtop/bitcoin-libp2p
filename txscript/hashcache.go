...
// from  O(N^2) to O(N).
func calcHashPrevOuts(tx *core.MsgTx) chainhash.Hash {
	var b bytes.Buffer
...
// hashing computation, reducing the complexity of validating SigHashAll inputs
// from O(N^2) to O(N).
func calcHashSequence(tx *core.MsgTx) chainhash.Hash {
	var b bytes.Buffer
	for _, in := range tx.TxIn {
		var buf [4]byte
...
func calcHashOutputs(tx *core.MsgTx) chainhash.Hash {
	var b bytes.Buffer
	for _, out := range tx.TxOut {
		core.WriteTxOut(&b, 0, 0, out)
	}
...
type PrevOutputFetcher interface {
	...
	FetchPrevOutput(core.OutPoint) *core.TxOut
}
...
func (c *CannedPrevOutputFetcher) FetchPrevOutput(core.OutPoint) *core.TxOut {
	return &core.TxOut{
		PkScript: c.pkScript,
		Value:    c.amt,
	}
...
type MultiPrevOutFetcher struct {
	prevOuts map[core.OutPoint]*core.TxOut
}
...
func NewMultiPrevOutFetcher(prevOuts map[core.OutPoint]*core.TxOut) *MultiPrevOutFetcher {
	if prevOuts == nil {
		prevOuts = make(map[core.OutPoint]*core.TxOut)
	}
...
func (m *MultiPrevOutFetcher) FetchPrevOutput(op core.OutPoint) *core.TxOut {
	return m.prevOuts[op]
}

// AddPrevOut adds a new prev out, tx out pair to the backing map.
func (m *MultiPrevOutFetcher) AddPrevOut(op core.OutPoint, txOut *core.TxOut) {
	m.prevOuts[op] = txOut
}
...
func calcHashInputAmounts(tx *core.MsgTx, inputFetcher PrevOutputFetcher) chainhash.Hash {
	var b bytes.Buffer
...
func calcHashInputScripts(tx *core.MsgTx, inputFetcher PrevOutputFetcher) chainhash.Hash {
	var b bytes.Buffer
	for _, txIn := range tx.TxIn {
		prevOut := inputFetcher.FetchPrevOutput(txIn.PreviousOutPoint)
...
// transaction.
func NewTxSigHashes(tx *core.MsgTx,
...
// transaction.
func (h *HashCache) AddSigHashes(tx *core.MsgTx, inputFetcher PrevOutputFetcher) {
	h.Lock()
