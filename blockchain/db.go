...
const (
	...
	utxoKeySize                  = 1 + 20 // 1字节地址类型 + 20字节hash160
)

// 将地址字符串转换为utils.Address
func (b *BlockChain) parseAddress(address string) (utils.Address, error) {
	// 解析地址字符串为utils.Address
	addr, err := utils.DecodeAddress(address, b.chainParams)
	if err != nil {
...
// 将地址转换为索引键，与AddrUtxoIndex.utxoKeyFromAddr保持一致
func (b *BlockChain) utxoKeyFromAddr(addr utils.Address) ([utxoKeySize]byte, error) {
	...
	return [utxoKeySize]byte{}, errors.New("不支持的地址类型")
}

// 生成UTXO索引的键，与AddrUtxoIndex.UtxoIndexKey保持一致
func (b *BlockChain) utxoIndexKey(addrKey [utxoKeySize]byte, txHash *chainhash.Hash, outIndex uint32) []byte {
...
// 与AddrUtxoIndex.GetAddressAllUTXO保持一致的实现逻辑
...
// GetAddressAllUTXOFull 查询指定地址的所有未花费UTXO，包含完整信息
// 与AddrUtxoIndex.GetAddressAllUTXOFull保持一致的实现逻辑
func (b *BlockChain) GetAddressAllUTXOFull(address string) ([]*UtxoEntryFull, error) {
	...
	for iter.First(); iter.Valid(); iter.Next() {
		...
		utxo, err := b.utxoCache.getUtxoByOutPointFromCache(&txHash, outIndex)
		if utxo == nil {
			utxo, err = b.getUTXOByOutPoint(&txHash, outIndex)
			if err != nil {
				// 如果UTXO已被花费，跳过
...
//	根据交易哈希和输出索引获取UTXO完整信息
//
// 与AddrUtxoIndex.getUTXOByOutPoint保持一致
func (b *BlockChain) getUTXOByOutPoint(txHash *chainhash.Hash, index uint32) (*UtxoEntry, error) {
	// 构造UTXO查询键
...
func (b *BlockChain) GetMainBlockHeaderByHeight(height int32) (*core.BlockHeader, error) {
	// 1. 输入参数合法性校验
	if height < 0 {
		return nil, fmt.Errorf("invalid height: must be non-negative (got %d)", height)
	}
...
// GetBlockBodyByHashList 通过哈希列表批量查询区块体
// 实现逻辑：遍历哈希列表，对每个哈希执行单区块体查询逻辑，最后汇总结果
func (b *BlockChain) GetBlockBodyByHashList(list []*chainhash.Hash) ([]*core.MsgBlockBody, error) {
	// 1. 参数合法性校验
	if list == nil {
		return nil, fmt.Errorf("invalid parameter: hash list is nil")
	}
	if len(list) < 0 {
		return nil, fmt.Errorf("invalid parameter: hash list is nil")
	}
	// 2. 初始化结果切片（预分配内存提升性能）
	bodies := make([]*core.MsgBlockBody, 0, len(list))
	// 3. 遍历哈希列表，批量获取区块体
	for index, hash := range list {
		// 3.1 单个哈希合法性校验
		if hash == nil {
			return nil, fmt.Errorf("invalid hash at index %d: hash is nil", index)
		}
		hashStr := hash.String() // 预存哈希字符串，用于错误信息
		// 3.2 构造区块体查询键（与SaveBlock和GetBlockByHash保持一致）
		bodyKey := db.BlockBodyBucket.Key(hash[:])
		var bodyBytes []byte
		// 3.3 从数据库读取区块体字节数据
		err := b.db.Get(bodyKey, func(value []byte) error {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to get block body (hash: %s, index: %d): %w", hashStr, index, err)
		}
		if bodyBytes == nil {
			return nil, fmt.Errorf("block body not found (hash: %s, index: %d)", hashStr, index)
		}
		// 3.4 区块体反序列化（与GetBlockByHash保持一致的序列化协议）
		var protoBody wire.ProtoMsgBlockBody
		if err := proto.Unmarshal(bodyBytes, &protoBody); err != nil {
			return nil, fmt.Errorf("failed to unmarshal block body (hash: %s, index: %d): %w", hashStr, index, err)
		}
		// 3.5 转换为core.MsgBlockBody类型
		body := &core.MsgBlockBody{}
		if err := body.FromProto(&protoBody); err != nil {
			return nil, fmt.Errorf("failed to convert proto body to core.MsgBlockBody (hash: %s, index: %d): %w", hashStr, index, err)
		}
		body.Hash = hash
		// 3.6 添加到结果集
...
func (b *BlockChain) GetHeaderByInterval(startHeight int32, endHeight int32, endHash []byte) ([]*core.BlockHeader, error) {
	// 2. 输入参数合法性校验（分层校验，提前拦截无效请求）
	// 2.1 校验高度非负性（区块链高度从0开始，无负高度）
	if startHeight < 0 {
		return nil, fmt.Errorf("invalid startHeight: must be non-negative (got %d)", startHeight)
	}
	if endHeight < 0 {
		return nil, fmt.Errorf("invalid endHeight: must be non-negative (got %d)", endHeight)
	}
	// 2.2 校验区间有效性（起始高度不能大于结束高度）
	if startHeight > endHeight {
		return nil, fmt.Errorf("invalid interval: startHeight %d is greater than endHeight %d", startHeight, endHeight)
	}
	// 2.3 校验区间不超出主链范围（结束高度不能超过当前主链最新高度）
	latestHeight, err := b.GetMainLatestHeight()
	if err != nil {
		return nil, fmt.Errorf("failed to get main chain latest height: %w", err)
	}
	if endHeight > latestHeight {
		return nil, fmt.Errorf("interval exceeds main chain range: endHeight %d > latest height %d", endHeight, latestHeight)
	}
	//获取结束高度 是否与校验Hash一致
	blockByHeight, err := b.GetMainBlockHeaderByHeight(endHeight)
	if err != nil {
		return nil, err
	}
	blockEndHash := blockByHeight.BlockHash()
	if !bytes.Equal(blockEndHash[:], endHash) {
		return nil, fmt.Errorf("校验Hash和结束区块不一致")
	}
	// 3. 预分配结果切片（已知区间长度，避免动态扩容开销，提升性能）
	intervalLength := endHeight - startHeight + 1
	blockHeaders := make([]*core.BlockHeader, 0, intervalLength)
	// 4. 遍历区间，逐个获取区块头（复用成熟方法，避免重复编码）
	for currentHeight := startHeight; currentHeight <= endHeight; currentHeight++ {
		header, err := b.GetMainBlockHeaderByHeight(currentHeight)
		if err != nil {
			// 包装错误上下文：明确标记失败高度，便于问题定位
			return nil, fmt.Errorf("failed to get block header at height %d: %w", currentHeight, err)
		}
		// 额外防御性校验：虽GetMainBlockHeaderByHeight已处理nil，但避免极端异常场景
		if header == nil {
			return nil, fmt.Errorf("unexpected nil header at height %d (main chain data may be corrupted)", currentHeight)
		}
		// 按高度升序加入结果列表（与循环顺序一致，保证输出有序）
...
// GetTxByBlockHashAndIndex 根据区块哈希和交易在区块内的索引获取对应的完整交易（core.MsgTx）
// 实现逻辑：
// 1. 通过区块哈希查询完整区块
// 2. 利用交易在区块内的索引（inBlockIndex）直接定位交易
// 3. 验证交易哈希一致性后返回
func (bc *BlockChain) GetTxByBlockHashAndIndex(blockHash *chainhash.Hash, inBlockIndex uint32) (*core.MsgTx, error) {
	// 1. 参数合法性校验
	if blockHash == nil {
		return nil, fmt.Errorf("invalid parameter: block hash is nil")
	}
	...
	block, err := bc.GetBlockByHash(blockHash)
	if err != nil {
		return nil, fmt.Errorf("failed to get block by hash %s: %w", blockHashStr, err)
	}
	if block == nil {
		return nil, fmt.Errorf("block not found (hash: %s)", blockHashStr)
	}
	...
	// 4.1 检查交易索引是否超出范围
	if inBlockIndex >= txCount {
		return nil, fmt.Errorf("transaction index %d out of range for block %s (total transactions: %d)",
			inBlockIndex, blockHashStr, txCount)
	}

	// 4.2 通过索引直接获取目标交易
	tx := transactions[inBlockIndex]
	if tx == nil {
		return nil, fmt.Errorf("transaction at index %d in block %s is nil", inBlockIndex, blockHashStr)
	}
	return tx, nil
}
...
func (b *BlockChain) GetUTXOByTxIdAndOutIndex(id string, index uint32) (UtxoEntry, error) {
	...
	var utxoKey = b.UTXOKey(*txHash, index)
...
// 获取当前节点的主链稀疏同步索引（最大跨度1024）
// 逻辑：从最新区块开始，每隔≤1024个区块取一个索引节点，覆盖至创世区块（高度0）
func (b *BlockChain) GetSyncIndex() (*wire.SyncIndex, error) {
	b.chainLock.RLock()
	defer b.chainLock.RUnlock()
	...
	// 防御性处理：主链未初始化
	if latestHeight < 0 {
		return &wire.SyncIndex{
			Index: []*wire.BlockIndex{},
		}, nil
	}

	// 步骤2：生成高度列表（0,1,2,4,8...，最大跨度不超过1024）
	var heights []int32
	if latestHeight >= 0 {
		heights = append(heights, 0) // 起始点为0
	}

	prevHeight := int32(0)
	nextHeight := int32(1)

	for nextHeight <= latestHeight {
		heights = append(heights, nextHeight)

		// 计算下一步增长：前期指数增长，当跨度即将超过1024时改为固定1024增长
		currentStep := nextHeight - prevHeight
		var nextStep int32

		if currentStep < 128 {
			nextStep = currentStep * 2 // 指数增长
			// 确保下一步跨度不超过1024
			if nextStep > 128 {
				nextStep = 128
			}
		} else {
			nextStep = 128 // 达到最大跨度后保持固定步长
		}

		// 计算下一个高度，同时防止溢出
		nextNextHeight := nextHeight + nextStep
		if nextNextHeight < nextHeight { // 检测到整数溢出
			break
		}
	...
	// 确保最新高度被包含（如果不在列表中）
	if len(heights) == 0 || heights[len(heights)-1] != latestHeight {
		heights = append(heights, latestHeight)
	}

	// 步骤3：按从高到低的顺序生成BlockIndex
	var blockIndexes []*wire.BlockIndex
	for i := len(heights) - 1; i >= 0; i-- {
		currentHeight := heights[i]

		// 获取对应高度的区块哈希
		blockHash, err := b.MainChainGetHashByHeightInit32(currentHeight)
		if err != nil {
			return nil, fmt.Errorf("获取高度%d的主链区块哈希失败: %w", currentHeight, err)
		}
		if blockHash == nil {
			return nil, fmt.Errorf("高度%d的主链区块哈希不存在（主链数据异常）", currentHeight)
		}
